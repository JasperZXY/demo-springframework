package org.ruanwei.demo.springframework.integration.jdbc.annotation;
...
import org.ruanwei.demo.springframework.integration.UserDAO;
 ...
 * <li>Propagation setting is PROPAGATION_REQUIRED.
 * <li>Isolation level is ISOLATION_DEFAULT.
 * <li>Transaction is read/write.
 * <li>Transaction timeout defaults to the default timeout of the underlying
 * transaction system, or to none if timeouts are not supported.
 * <li>Any RuntimeException triggers rollback, and any checked Exception does
 * not. The standard javax.transaction.Transactional annotation is also
 * supported.
 * 
 ...
 */
@Repository("userDAO")
@Transactional(readOnly = true, propagation = Propagation.REQUIRED, isolation = Isolation.DEFAULT, rollbackFor = UnsupportedOperationException.class)
...
//	});
	...
	public String findNameById(int id) {
		...
		String name = jdbcTemplate.queryForObject(sql, new Object[] { 1L }, String.class);
		return name;
	}
	...
	public String findNameById2(int id) {
		...
		String name = namedParameterJdbcTemplate.queryForObject(sql, namedParameters, String.class);
		return name;
	}
	...
	public User findUserById(int id) {
		...
		User user = jdbcTemplate.queryForObject(sql, new Object[] { 1L }, new RowMapper<User>() {
			@Override
	...
	public void findUserList2() {
		...
		if (jdbcTemplate != null) {
			...
			for (Map<String, Object> map : list) {
				String output = String.format("id=%d, name=%s, gender=%b", map.get("id"), map.get("name"),
						map.get("gender"));
				log.info(output);
			}
		}
